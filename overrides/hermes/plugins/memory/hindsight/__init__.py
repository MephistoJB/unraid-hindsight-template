...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def system_prompt_block(self) -> str:
        if self._memory_mode == "context":
            return (
                f"# Hindsight Memory\n"
                f"Active (context mode, {self._budget} recall budget).\n"
                f"Relevant memories are automatically injected into context."
            )
        if self._memory_mode == "tools":
            return (
                f"# Hindsight Memory\n"
                f"Active (tools mode, {self._budget} recall budget).\n"
                f"Use hindsight_recall to search, hindsight_reflect for synthesis, "
                f"hindsight_retain to store facts."
            )
        return (
            f"# Hindsight Memory\n"
            f"Active ({self._budget} recall budget).\n"
            f"Relevant memories are automatically injected into context. "
            f"Use hindsight_recall to search, hindsight_reflect for synthesis, "
            f"hindsight_retain to store facts."
        )
    ...
    def _recall_prefetch_text(self, query: str) -> str:
        """Return recall context for one query without mutating the cache."""
        ...
        # The 0.6.1 generated Python client drops the API's `scores` field.
        # Use the REST response when score filtering is enabled so irrelevant
        # personal memories are not injected into otherwise public requests.
        if self._recall_min_score > 0:
            try:
                ...
                url = (
                    f"{self._api_url.rstrip('/')}/v1/default/banks/"
                ...
                filtered = [
                    ...
                    if float((result.get("scores") or {}).get("final", 0.0))
                    >= self._recall_min_score
                ]
                logger.debug(
                    "Prefetch: score filter retained %d/%d results (minimum=%s)",
                    len(filtered),
                    len(results),
                    self._recall_min_score,
                )
                ...
                # Fall back to the unfiltered client response. This may route
                # more requests locally, but never drops memory on API drift.
                logger.debug(
                    "Prefetch: scored REST recall failed, using client: %s",
                    exc,
                    exc_info=True,
                )
        ...
        for result in resp.results or []:
            ...
            if (
                self._recall_min_score <= 0
                or final_score is None
                or float(final_score) >= self._recall_min_score
            ):
        ...
        return (
            "\n".join(
                f"- {result.text}" for result in filtered_results if result.text
            )
            if filtered_results
            else ""
        )
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        if self._prefetch_thread and self._prefetch_thread.is_alive():
            logger.debug("Prefetch: background recall still running")
            return ""
        with self._prefetch_lock:
            result = self._prefetch_result
            result_query = self._prefetch_query
            self._prefetch_result = ""
            self._prefetch_query = ""

        # API and WebUI requests may construct a fresh AIAgent for every turn.
        # In that case there is no post-turn background cache to consume. A
        # cache prepared for a different query is also not relevant to the
        # current turn, so fall back to one synchronous recall.
        if not result or result_query != query:
            try:
                result = self._recall_prefetch_text(query)
            except Exception as exc:
                logger.debug(
                    "Hindsight current-turn prefetch failed: %s",
                    exc,
                    exc_info=True,
                )
                result = ""
